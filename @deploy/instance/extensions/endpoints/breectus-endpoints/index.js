...
// @packages/breectus/src/endpoints/index.ts
...
var endpoints_default = defineEndpoint({
  id: "breectus",
  handler: (router, { env }) => {
    const jobsDir = (0, import_path.resolve)(env.EXTENSIONS_PATH, "jobs");
    router.get("/list-jobs", async (req, res) => {
      const fsJobs = await (0, import_fast_glob.default)(`${jobsDir}/*.js`);
      const bree = req.app.get("bree");
      ...
      const result = fsJobs.map((fsJobPath) => {
        ...
        if (breeJob) {
          const _j = breeJob;
          _j.status = "done";
          if (bree.workers.has(breeJob.name))
            _j.status = "active";
          else if (bree.timeouts.has(breeJob.name))
            _j.status = "delayed";
          else if (bree.intervals.has(breeJob.name))
            _j.status = "waiting";
          return _j;
        } else {
          return {
    ...
    router.post("/add-job/:jobName", async (req, res) => {
      ...
      const fsJobs = await (0, import_fast_glob.default)(`${jobsDir}/${jobName}.js`);
      if (fsJobs.length === 0)
